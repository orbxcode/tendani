...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`about_testimonials\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`about_testimonials_order_idx\` ON \`about_testimonials\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`about_testimonials_parent_id_idx\` ON \`about_testimonials\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "about_id", "media_id") SELECT "id", "order", "parent_id", "path", "users_id", "about_id", "media_id" FROM \`payload_locked_documents_rels\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`about_testimonials\`;`)
